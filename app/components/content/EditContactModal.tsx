...
export function EditContactModal({
  ...
  // Filtrar siglas basado en el input de sigla (únicas)
  ...
  // Enlace bidireccional al escribir coincidencias exactas
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen && contact) {
      ...
      // Agregar usuarios adicionales (solo para números fijos)
      if (contact.tipo === "Fijo" && contact.additionalContacts) {
        contact.additionalContacts.forEach((additional) => {
          if (additional.nombre) {
            usuarios.push({
      ...
      // Si no hay usuarios, agregar uno vacío
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.tipo === "Móvil" && numeroLimpio.length !== 8) {
      newErrors.numero = "El número móvil debe tener exactamente 8 dígitos";
    } else if (
      formData.tipo === "Fijo" &&
      (numeroLimpio.length < 3 || numeroLimpio.length > 4)
    ) {
      newErrors.numero = "El número fijo debe tener entre 3 y 4 dígitos";
    }
    ...
    // Validar ubicación para números fijos
    if (formData.tipo === "Fijo" && !formData.ubicacion.trim()) {
      newErrors.ubicacion = "La ubicación es requerida para números fijos";
    }

    // Validar usuarios
    const usuariosValidos = formData.usuarios.filter((usuario) =>
      usuario.nombre.trim()
    );
    if (usuariosValidos.length === 0) {
      newErrors.usuarios = ["Debe agregar al menos un usuario"];
    } else {
      // Validar que los usuarios móviles tengan cargo
      if (formData.tipo === "Móvil") {
        const usuariosSinCargo = usuariosValidos.filter(
          (usuario) => !usuario.cargo.trim()
        );
        if (usuariosSinCargo.length > 0) {
          newErrors.usuarios = ["El cargo es requerido para números móviles"];
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/contactos/${contact.id}`, {
        ...
        body: JSON.stringify({
          numero: formData.numero.replace(/[^0-9]/g, ""),
          tipo: formData.tipo,
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || "Error al actualizar el contacto");
      }
    } catch (error) {
  ...
  const handleUsuarioChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      usuarios: prev.usuarios.map((usuario, i) =>
        i === index ? { ...usuario, [field]: value } : usuario
      ),
    }));
  };
